...
### Requirement: Audio player (Browser TTS)

The system SHALL provide text-to-speech audio playback using the Web Speech API.

#### Scenario: Play audio via TTS

- **WHEN** a user clicks play on the audio player
- **THEN** the system uses the browser's speechSynthesis API to speak the lesson content

#### Scenario: Playback controls

- **WHEN** audio is playing via TTS
- **THEN** the system provides play/pause, speed control (0.5x-2x), and voice selection

#### Scenario: Text highlighting

- **WHEN** TTS is speaking
- **THEN** the system highlights the current sentence/paragraph being spoken

#### Scenario: Voice selection

- **WHEN** multiple TTS voices are available
- **THEN** the system allows the user to select their preferred voice

#### Scenario: Resume position

- **WHEN** a user returns to a partially-listened lesson
- **THEN** the system offers to resume from their last position (paragraph level)

#### Scenario: TTS unavailable

- **WHEN** the browser does not support Web Speech API
- **THEN** the system displays a notice and offers text-only mode
